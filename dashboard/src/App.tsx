...
const App: React.FC = () => {
  ...
  // WebSocket is used ONLY for connection status and circuit breaker events
  const { cbEvents, connected } = useWebSocket();

  // Poll data always drives the dashboard — respects the lookback window
  const { metrics, loading, error, refresh } = useMetricsPoll(15000, lookback);
